...
export default function CreatedBooking({ car }) {
        ...
        const fetchBookedSlots = async () => {
            ...
            try {
                ...
                const slots = response.data?.data?.map((bookingItem) => ({
                    start: dayjs(bookingItem.start_date).startOf("day"),
                    end: dayjs(bookingItem.end_date),
                    blockedEnd: dayjs(bookingItem.end_date).add(MAINTENANCE_DAYS, "day").endOf("day")
                })) || [];
    ...
    useEffect(() => {
        ...
        const hasOverlap = bookedSlots.some((slot) =>
            startDate.isBefore(slot.blockedEnd) && endDate.isAfter(slot.start)
        );

        if (hasOverlap) {
            setIsAvailable(false);
            setAvailabilityMessage("Khoảng thời gian này đã bị đặt hoặc đang trong thời gian bảo dưỡng 2 ngày sau khi trả xe.");
        } else {
            setIsAvailable(true);
    ...
    const isDateBlocked = (date) => {
        ...
        return bookedSlots.some((slot) =>
            ...
            currentDate.isSame(slot.blockedEnd, "day") ||
            (currentDate.isAfter(slot.start, "day") && currentDate.isBefore(slot.blockedEnd, "day"))
        );
    };
